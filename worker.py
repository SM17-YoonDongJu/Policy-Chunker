...
한 사이클 = ingest_many(신규/변경 문서만 doc_hash 멱등 적재) → rebuild_search_terms(BM25 용어).
두 CLI를 subprocess로 부른다 — 한 사이클이 죽어도 데몬은 살아 다음 주기를 계속한다.

환경변수:
  INGEST_INTERVAL_SECONDS  주기(초). 기본 604800(7일).
  MANIFEST_PATH            매니페스트 경로. 기본 /data/docs.yaml (호스트 ./data 마운트).
...
def _cycle() -> None:
    """인덱싱 1회. 매니페스트가 없으면 건너뛴다(데몬은 계속 떠 있는다)."""
    ...
    if not _run("ingest_many", [sys.executable, "ingest_many.py", "--manifest", _MANIFEST]):
...
def main() -> None:
    ...
    logger.info(f"인덱싱 데몬 시작 — 주기 {_INTERVAL}s ({_INTERVAL / 3600:.1f}h), 매니페스트 {_MANIFEST}")
